...
class Music:
    ...
    def url_list(self):
        """
        只获取音乐列表里的第一首歌曲
        :return: 返回这首歌曲的音乐信息和音乐外链
        """
        lists = self.res['data']['list']
        for i in lists:
            # 获取遍历出来的音乐信息
            rid = i['rid']
            artist = i['artist']
    ...
    def url_bytes(self):
        ...
        """
        res = requests.get(self.mp3_url).json()
    ...
    def mp3_open(self):
        ...
        :return:
        """
        conte_size = int(self.res_url.headers['content-length']) / 1024
        with open('{}.mp3'.format(self.info), 'wb') as f:
            # f.write(url_bytes)
            for i in tqdm(iterable=self.res_url.iter_content(1024),
                          total=conte_size,
                          unit='kb',
                          desc='下载中...'):
                f.write(i)
