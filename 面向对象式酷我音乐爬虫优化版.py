...
class Music:
    ...
    def __url_list(self, res):
        ...
        for i in lists:
            ...
            num += 1
            info = '{} {} {}'.format(num, artist, name)
            print(info)
            music_info = info + ',' + mp3_url
            
            dic[int(num)] = music_info
        music_name = input('请输入想要下载的歌曲序列号:')
        resp = dic[int(music_name)]
        #print(resp)
        name = resp.split(',')[0]
        url = resp.split(',')[-1]

        self.__url_bytes(name, url)
            
    def __url_bytes(self, name, url):
        """
        ...
        self.__mp3_open(name, res_url)
    def __mp3_open(self, name, res_url):
        """
        将数据转换成二进制数据然后写入本地
        :return:
        """
